            ...
            <div class="overflow-x-auto">
              <table class="w-full">
                ...
                <tbody class="divide-y divide-border">
                  @for (key of sortedSshKeys(); track trackByKeyId($index, key)) {
                    <tr
                          ...
                          @for (p of getProviderSyncStatus(key); track p.id) {
                            @if (!p.synced && !p.error) {
                              <button
                                ...
                                >
                                @if (p.logoUrl) {
                                  <img [src]="p.logoUrl" [alt]="p.displayName" class="w-4 h-4 object-contain opacity-40" />
                                }
                                @if (!p.logoUrl) {
                                  <ng-icon name="lucideServer" class="w-3 h-3"></ng-icon>
                                }
                                @if (isSyncing(key.id, p.id)) {
                                  <ng-icon name="lucideLoader" class="w-3 h-3 animate-spin"></ng-icon>
                                }
                                @if (!isSyncing(key.id, p.id)) {
                                  <ng-icon name="lucideRefreshCw" class="w-3 h-3"></ng-icon>
                                }
                              </button>
                            }
                            @if (p.error) {
                              <button
                                ...
                                >
                                @if (p.logoUrl) {
                                  <img [src]="p.logoUrl" [alt]="p.displayName" class="w-4 h-4 object-contain opacity-70" />
                                }
                                @if (!p.logoUrl) {
                                  <ng-icon name="lucideServer" class="w-3 h-3"></ng-icon>
                                }
                                ...
                                @if (!isSyncing(key.id, p.id)) {
                                  <ng-icon name="lucideTriangleAlert" class="w-3 h-3"></ng-icon>
                                }
                              </button>
                            }
                            @if (p.synced) {
                              <div class="inline-flex items-center rounded border border-green-500/30 bg-green-500/10 text-green-700 dark:text-green-400 overflow-hidden">
                                <span class="inline-flex items-center gap-1 px-2 py-1" [title]="p.displayName + ' — synced'">
                                  @if (p.logoUrl) {
                                    <img [src]="p.logoUrl" [alt]="p.displayName" class="w-4 h-4 object-contain" />
                                  }
                                  @if (!p.logoUrl) {
                                    <ng-icon name="lucideServer" class="w-3 h-3"></ng-icon>
                                  }
                  ...
                  @if (!isLoading() && sshKeys().length === 0 && !error()) {
                    <tr>
          ...
          <div class="p-6 space-y-4">
            <div class="bg-muted rounded-md p-3">
              <div class="flex items-center space-x-2 mb-1">
                ...
                <span class="font-medium text-foreground truncate" [title]="$safeNavigationMigration(keyToDelete?.name)">{{ keyToDelete?.name }}</span>
              </div>
              <div class="text-sm text-muted-foreground font-mono truncate" [title]="$safeNavigationMigration(keyToDelete?.fingerprint)">
            ...
            <div class="space-y-4">
              <div>
                ...
                <input
                  ...
                  />
                @if (addKeyForm.get('name')?.invalid && addKeyForm.get('name')?.touched) {
              ...
              <div>
                ...
                <input
                  ...
                  />
                @if (addKeyForm.get('userName')?.invalid && addKeyForm.get('userName')?.touched) {
                  <p class="text-sm text-destructive mt-1">
              ...
              @if (!addKeyForm.get('generateNew')?.value) {
                <div>
                  <label class="block text-sm font-medium text-foreground mb-2">
                    Public Key *
                  </label>
                  ...
                  @if (addKeyForm.get('publicKey')?.invalid && addKeyForm.get('publicKey')?.touched) {
                    <p class="text-sm text-destructive mt-1">
              ...
              @if (addKeyForm.get('generateNew')?.value) {
                <div class="bg-primary/10 border border-primary/20 rounded-md p-4">
                  <div class="flex">
              ...
              @if (!addKeyForm.get('generateNew')?.value) {
                <div class="bg-yellow-500/10 border border-yellow-500/20 rounded-md p-4">
                  <div class="flex">
                ...
                <div class="flex flex-wrap gap-2">
                  @for (p of enabledProviders(); track p.provider) {
                    ...
                    [class]="isProviderSelected(p.provider)
                      ...
                      >
                      @if (isProviderSelected(p.provider)) {
                        <ng-icon name="lucideCheck" class="w-3 h-3"></ng-icon>
                      }
                      {{ p.provider }}
                    </button>
