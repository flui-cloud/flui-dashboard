...
@Component({
  ...
  template: `
    <div class="p-6 space-y-6">
      ...
      <!-- Loading State con Pulse Effect -->
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-1.5 flex-wrap">
                      @for (p of getProviderSyncStatus(key); track p.id) {
                        ...
                        <!-- Sync error: warning badge, click to retry -->
                        @if (p.error) {
                          <button
                            (click)="syncKeyToProvider(key, p.id)"
        ...
        <div class="p-6 space-y-4">
          ...
          <div>
            ...
            @if (keyToDelete?.syncedFromProvider) {
              <!-- Provider-only key: provider is required, no choice -->
...
export class SshKeysComponent {
  ...
  // Track in-progress syncs/unsyncs as "keyId:providerId"
  ...
  constructor(private readonly fb: FormBuilder) {
    ...
    // Carica prima i provider, poi le SSH keys (per passare i provider attivi)
  ...
  /** Returns sync status for each active (configured) provider for this key */
  ...
  syncKeyToProvider(key: SSHKeyDto, providerId: string): void {
    ...
    this.accessManagementService.accessControllerUpdateSSHKey(key.id, dto, 'response').subscribe({
      next: (response) => {
        // 207 = partial success: some providers failed, body has syncErrors
  ...
  loadSshKeys(): void {
    ...
    const activeProviders = this.providersService.activeProviders()
      .map(c => c.provider.toLowerCase() as 'contabo' | 'hetzner' | 'scaleway');
  ...
  confirmBulkDelete(): void {
    ...
    const deleteNext = (index: number, failed: number): void => {
      ...
      // Provider-only keys always require their source provider regardless of scope
      let removeFromProviders: boolean;
      let provider: 'contabo' | 'hetzner' | 'scaleway' | undefined;
  ...
  deleteKey(key: SSHKeyDto): void {
    ...
    // For provider-only keys, the provider is required — pre-select it
    if (key.syncedFromProvider && key.source !== 'local') {
