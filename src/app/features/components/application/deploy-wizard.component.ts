    ...
    <div class="max-w-4xl mx-auto p-6">
      ...
      <div class="bg-card border border-border rounded-lg p-6 mb-6">
        @switch (currentStepId()) {
          ...
          <!-- Step: Analysis (Flow C only — repo Dockerfile status) -->
          @case ('analysis') {
            <div class="space-y-4">
              @switch (analysisBranch()) {
                @case ('loading') {
                  <div class="flex items-center justify-center py-10">
                    ...
                    <span class="ml-2 text-sm text-muted-foreground">
                      {{ isCheckingDockerfile() ? 'Checking for Dockerfile...' : 'Analyzing repository...' }}
                    </span>
                  </div>
                }

                @case ('flui-ready') {
                  @let da = analysisForSelector()!.dockerfileAnalysis!;
                  <div class="p-4 rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/10 space-y-3">
                    <div class="flex items-start gap-3">
                      ...
                      <div>
                        <p class="text-sm font-semibold text-green-800 dark:text-green-300">Flui Ready</p>
                        <p class="text-xs text-green-700 dark:text-green-400 mt-1">
                          This repository ships a Flui-managed Dockerfile. Flui trusts its runtime defaults and will build it as-is.
                        </p>
                      </div>
                    </div>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs pt-1">
                      @if (da.port) {
                        <div class="p-2 rounded bg-green-100/60 dark:bg-green-900/20">
                          ...
                          <div class="font-mono font-medium">{{ da.port }}</div>
                        </div>
                      }
                      @if (da.baseRuntime) {
                        <div class="p-2 rounded bg-green-100/60 dark:bg-green-900/20">
                          <div class="text-muted-foreground">Runtime</div>
                          <div class="font-medium">{{ da.baseRuntime }}</div>
                        </div>
                      }
                      @if (da.hasMultiStage) {
                        <div class="p-2 rounded bg-green-100/60 dark:bg-green-900/20">
                          <div class="text-muted-foreground">Layout</div>
                          <div class="font-medium">Multi-stage</div>
                        </div>
                      }
                    </div>
                  </div>
                ...
                @case ('custom-dockerfile') {
                  @let da = analysisForSelector()!.dockerfileAnalysis!;
                  <div class="p-4 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/10 space-y-3">
                    <div class="flex items-start gap-3">
                      <ng-icon name="lucideTriangleAlert" class="h-5 w-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                      <div>
                        <p class="text-sm font-semibold text-amber-900 dark:text-amber-100">Custom Dockerfile detected</p>
                        <p class="text-xs text-amber-700 dark:text-amber-300 mt-1">
                          This Dockerfile wasn't generated by Flui and may not build reliably. We <strong>strongly recommend</strong> starting from a Flui template instead.
                        </p>
                        <p class="text-xs text-amber-700 dark:text-amber-300 mt-1">
                          If you want to keep this Dockerfile, add <code class="font-mono bg-amber-100/70 dark:bg-amber-900/40 px-1 rounded">#flui-managed</code> on the first line as a trust marker, or proceed anyway and fill in port / healthcheck / resources manually.
                        </p>
                      </div>
                    </div>
                    @if (da.port || da.baseRuntime) {
                      <div class="flex flex-wrap gap-2 text-xs pt-1">
                        @if (da.port) {
                          <span class="px-2 py-0.5 rounded bg-amber-100/60 dark:bg-amber-900/20 text-muted-foreground">Detected port: <span class="font-mono font-medium text-foreground">{{ da.port }}</span></span>
                        }
                        @if (da.baseRuntime) {
                          <span class="px-2 py-0.5 rounded bg-amber-100/60 dark:bg-amber-900/20 text-muted-foreground">Runtime: <span class="font-medium text-foreground">{{ da.baseRuntime }}</span></span>
                        }
                      </div>
                    }
                    <div class="flex items-center gap-2 pt-2">
                      <button type="button" (click)="switchToTemplateFlow()"
                        ...
                        <ng-icon name="lucideRocket" class="h-3.5 w-3.5" /> Switch to template flow
                      </button>
                      <button type="button" (click)="proceedWithCustomDockerfile()"
                ...
                @case ('no-dockerfile') {
                  <div class="p-4 rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/10 space-y-3">
                    <div class="flex items-start gap-3">
                      ...
                      <div>
                        <p class="text-sm font-semibold text-red-800 dark:text-red-300">No Dockerfile found</p>
                        <p class="text-xs text-red-700 dark:text-red-400 mt-1">
                          Flui needs a Dockerfile to build this repository. The fastest path is to start from a Flui template — they come pre-wired with a production-ready Dockerfile.
                        </p>
                      </div>
                    </div>
                    <div class="flex items-center gap-2 pt-2">
                      <button type="button" (click)="switchToTemplateFlow()"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-primary text-primary-foreground text-xs font-medium hover:bg-primary/90 transition-colors">
                        <ng-icon name="lucideRocket" class="h-3.5 w-3.5" /> Use a template
                      </button>
                      <button type="button" (click)="switchToDockerImage()"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-border text-xs font-medium hover:bg-accent transition-colors">
                        <ng-icon name="lucideContainer" class="h-3.5 w-3.5" /> Use a pre-built image
                      </button>
                    </div>
                  </div>
                }
                ...
                @case ('analyze-failed') {
                  <div class="p-4 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/10 space-y-3">
                    <div class="flex items-start gap-3">
                      <ng-icon name="lucideTriangleAlert" class="h-5 w-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                      <div>
                        <p class="text-sm font-semibold text-amber-900 dark:text-amber-100">Could not analyze Dockerfile</p>
                        <p class="text-xs text-amber-700 dark:text-amber-300 mt-1">
                          Proceeding will trust the repository's Dockerfile as-is. You'll need to fill in port, healthcheck and resources manually.
                        </p>
                      </div>
                    </div>
                    <div class="flex items-center gap-2 pt-2">
                      <button type="button" (click)="triggerBuildPathAnalysis()"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-border text-xs font-medium hover:bg-accent transition-colors">
          ...
          <!-- Step: Flui Config (Flow C, after analysis — only shown when not Flui-managed) -->
          @case ('flui-config') {
            <div class="space-y-5">
              <div>
                <h3 class="text-base font-semibold mb-1">Application configuration</h3>
                <p class="text-sm text-muted-foreground">
                  @if (state.defaultsSource() === 'dockerfile-analysis') {
                    These defaults come from your Flui-managed Dockerfile. Override only if you know what you're doing.
                  } @else {
                    No reliable defaults are available for this repo. Please fill in the values below.
                  }
                </p>
              </div>
              ...
              <!-- Port -->
              <div>
                <label class="text-sm font-medium block mb-1.5">Application Port <span class="text-destructive">*</span></label>
                <input
                  type="number"
                  [value]="state.deployConfig().port || ''"
                  (input)="updateFluiConfigPort($any($event.target).value)"
                  placeholder="e.g. 3000"
                  min="1"
                  max="65535"
                  class="h-10 w-40 rounded-md border border-input bg-background px-3 text-sm"
                />
                <p class="text-xs text-muted-foreground mt-1">The port your application listens on inside the container.</p>
              </div>

              <!-- Healthcheck path -->
              <div>
                <label class="text-sm font-medium block mb-1.5">Healthcheck path</label>
                <input
                  type="text"
                  [value]="state.deployConfig().healthcheckPath"
                  (input)="updateFluiConfigHealthcheck($any($event.target).value)"
                  placeholder="/health"
                  class="h-10 w-full max-w-sm rounded-md border border-input bg-background px-3 text-sm font-mono"
                />
              </div>

              <!-- Resource profile -->
              <div>
                <label class="text-sm font-medium block mb-2">Resource profile</label>
                <div class="flex flex-wrap gap-2">
                  @for (p of ['nano','small','medium','large','xlarge']; track p) {
                    <button type="button" (click)="updateFluiConfigProfile(p)"
                      [class]="state.deployConfig().resourceProfile === p ? 'px-3 py-1.5 rounded-md border border-primary bg-primary/10 text-sm capitalize' : 'px-3 py-1.5 rounded-md border border-border text-sm text-muted-foreground hover:bg-accent capitalize'">
                      {{ p }}
                    </button>
                  }
                </div>
              ...
              <div>
                ...
                <input
                  type="number"
                  [value]="state.deployConfig().minReplicas"
                  (input)="updateFluiConfigReplicas($any($event.target).value)"
                  min="1"
                  max="20"
                  class="h-10 w-32 rounded-md border border-input bg-background px-3 text-sm"
                />
              </div>
            </div>
          }
                  ...
                  <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between">
                      ...
                      <span class="font-medium">
                        ...
                        @if (state.defaultsSource() === 'dockerfile-analysis') {
                          <span class="ml-1 text-[10px] px-1 rounded bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">from Dockerfile</span>
...
export class DeployWizardComponent implements OnInit {
  ...
  // Analysis branch computed (Flow C)
  analysisFailed = signal<boolean>(false);
  ...
  // Build path analysis (git_build only)
  isAnalyzingForSelector = signal<boolean>(false);
  analysisForSelector = signal<RepositoryAnalysisDto | null>(null);
  /** Kept for back-compat with deploy dispatch: always 'github-actions' in the unified flow. */
  selectedBuildPath = signal<'github-actions' | 'railpack' | null>(null);

  /**
   * 4-branch computed state for the Flow C analysis step.
   * See docs/application-deployment/FRONTEND_TEMPLATE_DEPLOY.md for semantics.
   */
  readonly analysisBranch = computed<'loading' | 'flui-ready' | 'custom-dockerfile' | 'no-dockerfile' | 'analyze-failed'>(() => {
    if (this.isCheckingDockerfile() || this.isAnalyzingForSelector()) return 'loading';
    if (this.analysisFailed()) return 'analyze-failed';
    const check = this.dockerfileCheckResult();
    if (!check) return 'loading';
    if (!check.hasDockerfile) return 'no-dockerfile';
    const analysis = this.analysisForSelector();
    if (!analysis?.dockerfileAnalysis) return 'analyze-failed';
    return analysis.dockerfileAnalysis.isFluiManaged ? 'flui-ready' : 'custom-dockerfile';
  });
  ...
  readonly steps = computed<WizardStepperStep[]>(() => {
    ...
    if (sub === 'existing-repo') {
      ...
      const analysisValid = this.analysisBranch() === 'flui-ready' || !!this.selectedBuildPath();
      ...
      // Deploy config step only when we have no source of truth
      if (this.state.defaultsSource() !== 'template') {
        stepsC.push({
          id: 'flui-config',
          title: 'Config',
          icon: 'lucideSettings',
          isValid: this.state.deployConfig().port > 0,
          isCompleted: false,
        });
      }
    ...
    void (async () => {
      ...
      if (catalogSlug) {
        ...
        if (repo) {
          ...
          this.currentStepIndex.set(1);
          this.loadBranches().then(() => this.triggerBuildPathAnalysis());
        }
      }
    })();
  }
  ...
  nextStep(): void {
    ...
    // Flow C — kick off analysis when entering the analysis step
    if (nextStepId === 'analysis' && this.flowSubtype() === 'existing-repo') {
      this.loadBranches().then(() => this.triggerBuildPathAnalysis());
    }

    // Flow C — sync state service with repo info when entering cluster step from analysis
    if (nextStepId === 'cluster' && this.flowSubtype() === 'existing-repo') {
      const repo = this.selectedRepo();
      const analysis = this.analysisForSelector();
      if (repo && analysis) {
        this.state.initialize(repo.id, this.selectedBranch(), analysis, this.selectedCluster()?.id ?? '');
      }
    }
    ...
    // Flow C — extract env vars when entering env-vars step
  ...
  navigateToTemplates(): void {
    const framework = this.analysisForSelector()?.detection?.framework;
    this.router.navigate(['/apps/templates'], {
      queryParams: framework ? { framework } : undefined,
    });
  }
  ...
  selectFlow(sub: 'image' | 'template' | 'existing-repo' | 'marketplace'): void {
    ...
    this.analysisForSelector.set(null);
    this.dockerfileCheckResult.set(null);
    this.selectedBuildPath.set(null);
  ...
  /** User clicked "Proceed anyway" in custom-dockerfile or analyze-failed branch. */
  proceedWithCustomDockerfile(): void {
    this.selectedBuildPath.set('github-actions');
    // Sync state service with repo info so downstream steps have what they need
    const repo = this.selectedRepo();
    const analysis = this.analysisForSelector();
    if (repo) {
      this.state.initialize(repo.id, this.selectedBranch(), analysis ?? { repositoryId: repo.id, branch: this.selectedBranch() } as any, this.selectedCluster()?.id ?? '');
    }
    // Advance to next step
    this.currentStepIndex.update(i => i + 1);
  }

  // ===== Flow C — Flui Config step handlers =====

  updateFluiConfigPort(value: string): void {
    const port = +value || 0;
    this.state.deployConfig.update(c => ({ ...c, port }));
  }

  updateFluiConfigHealthcheck(value: string): void {
    this.state.deployConfig.update(c => ({ ...c, healthcheckPath: value }));
  }

  updateFluiConfigProfile(profile: string): void {
    this.state.deployConfig.update(c => ({ ...c, resourceProfile: profile as any }));
  }

  updateFluiConfigReplicas(value: string): void {
    const n = Math.max(1, +value || 1);
    this.state.deployConfig.update(c => ({ ...c, minReplicas: n, maxReplicas: Math.max(n, c.maxReplicas) }));
  }
  ...
  // V3: Dockerfile check signal
  dockerfileCheckResult = signal<{ hasDockerfile: boolean } | null>(null);
  isCheckingDockerfile = signal<boolean>(false);

  /**
   * Flow C analysis: check Dockerfile, analyze repo, derive the analysis branch.
   * Auto-selects `github-actions` only for the `flui-ready` branch — the other
   * branches require an explicit "Proceed anyway" click from the user.
   *
   * We also pre-load the template catalog in parallel because the subsequent
   * call to `state.initialize()` (fired when the user clicks Next) reads it
   * synchronously to derive port/healthcheck defaults for the detected framework.
   * Without this prefetch a race condition can leave deployConfig with a bare
   * /health default and crash the pod at the readiness probe.
   */
  async triggerBuildPathAnalysis(): Promise<void> {
    const repo = this.selectedRepo();
    if (!repo) return;

    this.isAnalyzingForSelector.set(true);
    this.analysisForSelector.set(null);
    this.selectedBuildPath.set(null);
    this.analysisFailed.set(false);
    this.dockerfileCheckResult.set(null);
    this.isCheckingDockerfile.set(true);

    // Kick off the template catalog load in parallel with the dockerfile check.
    // We'll await it below before returning so that by the time the spinner
    // clears, state.initialize() can synchronously look up framework defaults.
    const templatesLoaded = this.loadTemplates();

    try {
      const dockerCheck = await this.repoService.checkDockerfile(repo.id);
      this.dockerfileCheckResult.set(dockerCheck);
      this.isCheckingDockerfile.set(false);

      if (!dockerCheck.hasDockerfile) {
        // no-dockerfile branch — user must click a CTA to leave this step
        return;
      }

      const result = await this.repoService.analyzeRepository(repo.id, this.selectedBranch());
      this.analysisForSelector.set(result);

      if (!result?.dockerfileAnalysis) {
        this.analysisFailed.set(true);
        return;
      }

      if (result.dockerfileAnalysis.isFluiManaged) {
        // flui-ready — auto-select, Continue is enabled
        this.selectedBuildPath.set('github-actions');
      }
      // custom-dockerfile — leave selectedBuildPath null, user must Proceed anyway
    } catch {
      this.isCheckingDockerfile.set(false);
      if (this.dockerfileCheckResult()?.hasDockerfile) {
        this.analysisFailed.set(true);
      }
    } finally {
      // Wait for the template catalog before clearing the spinner — the next
      // step's state.initialize() reads it synchronously to derive framework
      // defaults for port/healthcheck. Keeping the spinner up here avoids a
      // race where the user clicks Next before templates have loaded.
      try { await templatesLoaded; } catch { /* non-fatal */ }
      this.isAnalyzingForSelector.set(false);
    }
  }
  ...
  switchToDockerImage(): void {
    this.selectFlow('image');
