...
/**
 * Grade Gauge Component
 * Displays a semicircular gauge chart (180°) for metric visualization
 * Similar to a speedometer or grade meter
 *
 * @example
 * ```html
 * <app-grade-gauge
 *   [data]="{value: 85, title: 'Memory Usage', subtitle: '8.5 GB / 10 GB'}"
 *   [config]="{unit: '%', thresholds: {warning: 75, danger: 90}}"
 * />
 * ```
 */
@Component({
  ...
  template: `
    <div class="grade-gauge-container" [style.height]="config().height">
      <div echarts
           [options]="chartOption()"
           [theme]="currentTheme()"
           class="w-full h-full">
...
export class GradeGaugeComponent implements OnDestroy {
  // Inputs
